...
public:
  ...
  public:
    ...
    Iterator(std::tuple<SparseSet<Components>*...> sets, const std::vector<Entity>* smallestEntities, std::size_t index)
        : sets_{sets}, smallestEntities_{smallestEntities}, index_{index} {
      advanceToValid();
    }
  ...
  explicit View(SparseSet<Components>*... sets) : sets_{sets...} {
    if (((sets == nullptr) || ...)) {
  ...
  Iterator begin() const {
    if (!smallestSetEntities_) return end();
...
public:
  ...
  void destroyEntity(Entity entity) {
    ...
    for (auto& sparseSet : sparseSets_) {
      if (sparseSet) sparseSet->destroyComponent(entity);
    }
  }
