...
GLuint Shader::createShader(GLenum type, const std::string& path) {
...
GLuint Shader::compileShader(GLenum type, const char* source, const std::string& shaderPath) const {
...
GLint Shader::getUniformLocation(const std::string& name) const noexcept {
  if (auto it{uniformLocations_.find(name)}; it != uniformLocations_.end()) return it->second;
